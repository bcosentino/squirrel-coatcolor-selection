...
win.data <- list(yg.occ = y.occ.g, ym.occ =y.occ.m, 
                 ...
                 Xdate = seq(min(c(date.occ.s, date.ct.s), na.rm=T), max(c(date.occ.s, date.ct.s), na.rm=T), length.out=100))
str(win.data)
...
cat("
    ...
    for(k in 1:100) {
      ...
      #melanic morph
      logit(pdet.m.pred[k]) <- alpha0[1] + alpha1[1]*Xdate[k] + alpha2[1]*pow(Xdate[k], 2)
      
      #gray morph
      logit(pdet.g.pred[k]) <- alpha0[2] + alpha1[2]*Xdate[k] + alpha2[2]*pow(Xdate[k], 2)
    }
...
model.sub.out <- jags(win.data, inits, params, "cline_subsetModel.txt", n.chains=nc, n.thin = nt, n.iter = ni, n.burnin = nb)
